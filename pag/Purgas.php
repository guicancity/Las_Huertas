...
<head>
	...
	?>	
	<title>Las huertas || Purgas</title>
		...
		<form  method="POST">
			<div class="row">
				<div class="col-10 text-center mt-4">
					<h3>Registro de purgas por animal</h3>
				</div>
			</div>
			<div class="row my-4">
			...
			<div class="row my-4">
				<div class="col-3 text-right">
					<h5>Purga aplicada:</h5>
				</div>
				<div class="col-6">
					<select id="sltPurga" class="custom-select" >
						<?php 
						
						$sql = mysqli_query($conexion,"SELECT IDPRODUCTOS, NOMBRE FROM PRODUCTOS WHERE TIPO = 2");
	...
	<script>
		$(function(){
			$('#btnAccion').on('click',function(e){
				e.preventDefault();
				const accion = 'seleccionaAnimalesActivos';
				//registraPurga
